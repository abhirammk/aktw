...
(* This package is an improvement over the package of Matt Headrick. It is orders of magnitude faster.
 *)

dim::usage =		"dim is the dimensionality of the space (number of coordinates)"
...
FullTensorQ[ expr_ ] := FullTensorQ[ expr, dim ]
...
(*If[
   !(SymmetricMatrixQ[metric] && (Length[metric] == dim)),
   Print[
...
inverse = Inverse[ met ] // FullSimplify
...
partial[ tensor_ ] := Map[ D[tensor,#]&, xmu ] //Simplify
...
divergence[ vector_?ArrayQ ] := Inner[D,vector,xmu,Plus,1] + Chrfel.vector //Simplify
...
covariant[ tensor_, indexp:{(up|down|none)...} ] := With[
  { trank = Length[indexp],
    Christoffelt = Transpose[ ch, {3,2,1} ]
    },
  partial[tensor] +
    Sum[
      Which[
        indexp[[index]] === down,
        - Transpose[
          Inner[ Times, tensor, ch, Plus, index ],
...
zeroTensor[ trank_Integer ] := ConstantArray[ 0, ConstantArray[ dim, trank ] ]

rg := (
       Unprotect[rg];
       rg = (
             detmet = Det[met] //Simplify;
...
lct := (
               Unprotect[lct];
               lct = rg Normal[ LeviCivitaTensor[ dim ] ];
               Protect[lct];
               lct
               )
...
Einstein := (
             Unprotect[Einstein];
             Einstein = RicciTensor - RicciScalar met / 2 // Simplify;
             Protect[ Einstein ];
             Einstein
             )
...
Weyl := (
         ...
         Weyl = (
                 lower[ Riemann, {4} ] +
                 antisymmetrize[
                                Transpose[ met ** (RicciScalar met/(dim-1) - 2 RicciTensor), {1,3,4,2} ],
                                {1,2}, {3,4}
                                ] 2 / (dim-2)
                 ) // Simplify;
         Protect[Weyl];
...
If[
   ...
   Cotton := (
              ...
              contract[
                       LeviCivita **
                       antisymmetrize[ covariant[RicciTensor-RicciScalar met/4], {1,2} ],
                       {1,4}, {2,5}
                       ] // Simplify
              )
   ]
...
WedgeExp[ expr_ ] := With[
                          ...
                          Exp[ scalarpart ] *
                          Sum[ 1/n! WedgePower[ Simplify[expr-scalarpart], n ], { n, 0, dim } ]
                          ]
...
HodgeStar[ dd[coords__] ] := (
                              rg Total[
                                       (
                                        ...
                                        ( dd @@ # )
                                        & ) /@
                                       Subsets[ coord, { dim - Length[{coords}] } ]
                                       ]
                              ) // Simplify
...
HodgeStarPolchinski[ form_?FullTensorQ ]:= With[
                                                ...
                                                contract[
                                                         form ** LeviCivita,
                                                         Sequence @@ Transpose[ {Range[trank],Range[dim+1,dim+trank]} ]
                                                         ] / trank!
                                                ]
...
antisymmetrize[
               Normal[
                      SparseArray[
                                  ...
                                  ConstantArray[ dim, Length[{coords}] ]
                                  ]
                      ]
               ]
...
hypersurface[ co_?(MemberQ[coord,#]&), signature:(1|-1):1 ] := With[
                                                                    ...
                                                                    unitnormal =
                                                                    Simplify[(signature inverse[[conum,conum]])^(-1/2)] UnitVector[dim,conum];
                                                                    projector = met - signature unitnormal ** unitnormal // Simplify;
                                                                    extrinsic = (1/2) Lie[ raise[unitnormal], projector ];
                                                                    extrinsictrace = contract[ extrinsic ];
                                                                    induced = Drop[ met, {conum}, {conum} ];
                                                                    display[ tensor_?ArrayQ ] /; (Union[Dimensions[tensor]]=={dim-1}) :=
                                                                    display[ tensor, HScoord ];
                                                                    HScoord = DeleteCases[coord,co];
                                                                    HSNameToNumber = Thread[ HScoord -> Range[dim-1] ];
                                                                    HScoordQ[ {} ] = False;
                                                                    ...
                                                                    ( Dimensions[tensor][[ Length[{indices1}] + 1 ]] == dim-1 );
